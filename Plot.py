...
modelname = 'Conformer9_2_CR=2,norescov'
model = Conformer(Device='cpu', depth=9, trans2con=True, channel_ratio=2)
...
for snr in sorted(SNRS):
    # extract classes @ SNR
    snrid = [a for a in testidx if dataset.y[a][1] == snr]
    Testx = []
    for i in snrid:
    ...
    # estimate classes
    PredY = model(Testx_tensor)
    print(PredY[0],Testy[0])
    ...
    plot_confusion_matrix(confnorm, labels=MODULATIONS.keys(), title=f"Confusion Matrix(SNR=%d)"%(snr), SNR=snr)
    cor = np.sum(np.diag(conf))
    ncor = np.sum(conf) - cor
    print ("Overall Accuracy: ", cor / (cor+ncor))
    acc[snr] = 100*cor/(cor+ncor)
# acc_df = pd.DataFrame(index=sorted(SNRS),columns=['acc'])
# for i in sorted(SNRS):
#     acc_df['acc'][i] = acc[i]
# acc_df.to_csv(csv_acc)
# sub_Acc.to_csv(csv_path)
